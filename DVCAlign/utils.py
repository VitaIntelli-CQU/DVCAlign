...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='DVCAlign', random_seed=666):
    """\
    ...
    if used_obsm not in adata.obsm:
        ...
        raise KeyError(
            f"Embedding '{used_obsm}' was not found in adata.obsm. "
            f"Available embeddings: {available}. "
            "Run DVCAlign.train_DVCAlign(..., key_added='DVCAlign') first, "
            "or pass used_obsm as one of the available keys."
        )
...
def ICP_align(adata_concat, adata_target, adata_ref, slice_target, slice_ref, landmark_domain,
              plot_align=False, embedding_key='DVCAlign'):
    ...
    mnn_dict = create_dictionary_mnn(
        batch_pair, use_rep=embedding_key, batch_name='batch_name', k=1, iter_comb=None, verbose=0
    )
    ...
    anchor_arr = adata_concat.obsm[embedding_key][anchor_ind, ]
    positive_arr = adata_concat.obsm[embedding_key][positive_ind, ]
...
def nn(ds1, ds2, names1, names2, knn=50, metric_p=2):
    nn_ = sklearn.neighbors.NearestNeighbors(knn, p=metric_p)
    nn_.fit(ds2)
